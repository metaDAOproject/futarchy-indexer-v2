...
async function setLatestTxSigProcessed(signature: string, programId: string) {
  try {
    logger.info(`setting latestTxSigProcessed to ${signature} for programId: ${programId}`);
    await db.update(schema.indexers).set({ latestTxSigProcessed: signature }).where(eq(schema.indexers.name, programId)).execute(); //here
  } catch (e) {
...
const programIds = [ V5_LAUNCHPAD_PROGRAM_ID, V5_AMM_PROGRAM_ID, V5_AUTOCRAT_PROGRAM_ID, V4_CONDITIONAL_VAULT_PROGRAM_ID, TOKEN_MIGRATOR_PROGRAM_ID]; 
