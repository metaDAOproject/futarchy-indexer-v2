...
import { updateOrInsertTokenBalance } from "./utils";
...
const IGNORE_VAULTS = process.env.V6_VAULT_BLACKLIST_DASHBOARDS 
  ? process.env.V6_VAULT_BLACKLIST_DASHBOARDS.split(',') 
  : [];
...
async function mapMintsToDAOs(conditionalMints: Set<string>): Promise<Map<string, 
  PublicKey[]>> {
    logger.info("Fetching all DAOs with embedded AMMs...");

    // Get DAOs from chain (they contain embedded AMM data)
    const allDaoAccounts = await futarchyClient.autocrat.account.dao.all();

    // Create a mapping of mint address to DAOs that use it
    const mintToDAOsMap = new Map<string, PublicKey[]>();

    for (const daoData of allDaoAccounts) {
      const dao = daoData.account;
      const baseMint = dao.baseMint.toString();
      const quoteMint = dao.quoteMint.toString();

      // If this DAO uses one of our conditional mints, add it to the map
      if (conditionalMints.has(baseMint)) {
        if (!mintToDAOsMap.has(baseMint)) {
          mintToDAOsMap.set(baseMint, []);
        }
        mintToDAOsMap.get(baseMint)?.push(daoData.publicKey);
      }

      if (conditionalMints.has(quoteMint)) {
        if (!mintToDAOsMap.has(quoteMint)) {
          mintToDAOsMap.set(quoteMint, []);
        }
        mintToDAOsMap.get(quoteMint)?.push(daoData.publicKey);
      }
    }

    logger.info(`Mapped ${mintToDAOsMap.size} mints to their DAOs`);
    return mintToDAOsMap;
  }

 async function getUsersForDAOs(daos: PublicKey[]): Promise<Map<string, string[]>> {
    const daoToUsersMap = new Map<string, string[]>();

    for (const daoPubkey of daos) {
      const daoAddr = daoPubkey.toString();

      // Get users from both conditional and spot swaps for this DAO
      const [conditionalUsers, spotUsers] = await Promise.all([
        db.selectDistinct({ userAddr: schema.v0_6_conditional_swaps.userAddr })
          .from(schema.v0_6_conditional_swaps)
          .where(eq(schema.v0_6_conditional_swaps.daoAddr, daoAddr)),

        db.selectDistinct({ userAddr: schema.v0_6_spot_swaps.userAddr })
          .from(schema.v0_6_spot_swaps)
          .where(eq(schema.v0_6_spot_swaps.daoAddr, daoAddr))
      ]);

      // Combine users from both swap types
      const allUsers = new Set([
        ...conditionalUsers.map(u => u.userAddr),
        ...spotUsers.map(u => u.userAddr)
      ]);

      // Filter out null/undefined and store
      const validUsers = Array.from(allUsers).filter((addr): addr is string =>
        addr !== null && addr !== undefined
      );

      daoToUsersMap.set(daoAddr, validUsers);
    }

    return daoToUsersMap;
  }

async function getMintUsersMapping(conditionalMints: Set<string>): Promise<Map<string, Map<string, string[]>>> { 
  // First get mint -> DAOs mapping
  const mintToDaosMap = await mapMintsToDAOs(conditionalMints);
  
  // Initialize result: mint -> (DAO -> users)
  const mintToDaoToUsersMap = new Map<string, Map<string, string[]>>();
  
  // For each mint, get its DAOs and their users
  for (const [mint, daos] of mintToDaosMap.entries()) {
    // Get the mapping of DAOs to their users
    const daoToUsersMap = await getUsersForDAOs(daos);
    
    // Store this mapping for the current mint
    mintToDaoToUsersMap.set(mint, daoToUsersMap);
  }
  
  logger.info(`Created mapping of ${mintToDaoToUsersMap.size} mints to their DAOs and users`);
  return mintToDaoToUsersMap;
}
...
async function processUserMintTokenAccounts( 
  mintToDaoToUsersMap: Map<string, Map<string, string[]>>,
  ...
  for (const [mintStr, daoToUsersMap] of mintToDaoToUsersMap.entries()) {
    const mintPubkey = conditionalMintPubkeys.get(mintStr);
    if (!mintPubkey) continue;
    
    logger.info(`Processing mint ${mintStr}`);
    
    // Collect all unique users across all DAOs for this mint
    const uniqueUsers = new Set<string>();
    for (const users of daoToUsersMap.values()) {
...
export async function captureTokenBalanceSnapshotV6(): Promise<{message: string, error: Error | undefined}> { 
  try {
    ...
    const mintToDaoToUsersMap = await getMintUsersMapping(conditionalMints);
    
    const result = await processUserMintTokenAccounts(mintToDaoToUsersMap, conditionalMintPubkeys);
